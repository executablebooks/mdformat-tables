...
SPECTESTS_PATH = next(TEST_DATA_DIR.glob("gfm_tables_spec.commit-*.json"))
...
def test_spec(entry):
    """Test mdformat-tables against the GFM spec.
    ...
    """
    if entry["name"] == "200":
        pytest.xfail("Need to fix this in markdown-it-py!")
    md_original = entry["md"]
    md_new = mdformat.text(md_original, extensions={"tables"})
    md_2nd_pass = mdformat.text(md_new, extensions={"tables"})
    assert is_md_equal(md_original, md_new, extensions={"tables"})
